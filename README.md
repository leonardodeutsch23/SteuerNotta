...
checks the native HTML constraints, displays translated inline feedback for every invalid control and
opens a frontend confirmation modal only when all constraints pass; neither action saves or
...
| Form Review and Reset actions | Inline feedback identifies all invalid controls, native constraints gate the local modal and Reset restores initial values |
...
- Native number, select and file controls may look slightly different between browsers.
...
1. Add conditional questions and more field-specific guidance if the form scope expands.
